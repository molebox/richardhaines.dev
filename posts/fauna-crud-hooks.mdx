...
locally and they work. They provide simple abstractions.
[Fauna Query Language (FQL)](https://docs.fauna.com/fauna/current/api/fql/)
enables so much more and the hooks will need refactoring but for a first version
im relatively pleased! 😃
    ...
    const update = async () => {
      ...
      try {
        client
          .query(
            q.Update(
              q.Select(["ref"], q.Get(q.Match(q.Index(collectionIndex), id))),
              {
                data
              }
            )
